...
def get_current_time() -> dict: # custom tool
    """ Get the current time and return it in the form of a dictionary.""" # docstring written so that the agent knows what the tool can be used for.
    return {
...
root_agent = Agent(
    ...
    tools=[factorial, get_current_time] # we can pass in multiple custom built tools
