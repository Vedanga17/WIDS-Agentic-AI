...
class PipelineState(TypedDict):
    ...
    # Prior turns of the conversation, oldest first, so follow-up questions can be
    # resolved (e.g. "what about the other one?"). Populated by the caller (app.py)
    # from its own chat history - the graph itself does not persist this between
    # invocations, since each run_query() call builds a fresh state.
